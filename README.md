# 자료구조
이 레포는 자료구조의 개념과 구현 코드가 저장된 공간입니다.        
...
* [우선순위 큐](https://github.com/whatsgoodg/Data-Structure/blob/main/README.md#priority-queue)
* [힙]()
...
<br><br><br>
# Priority Queue                     
**`Priority Queue`** 란 일반적으로 먼저 들어온 원소가 먼저 삭제되는 queue와 달리 우선순위대로 들어온 원소가 먼저 삭제되는 자료구조를 의미한다.                
우선순위 큐는 entry를 저장한다. 대개 entry는 (key, value) 형태로 사용하며, 우선순위는 key에게 적용된다.                            
...
>* 배열을 정렬할 때, compare 함수의 매개변수 a는 앞쪽, b는 뒷쪽에 위치할 원소이다. 이 함수가 정렬되고 싶은 조건에 true를 반환하게 구현하면 된다.(내림차순 a<b 반환)                
